...
#### Ответ
В случае успеха вернет объект с полями:
* ```isBanned``` - Забанен ли юзер. Принимает значения ```true``` или ```false```
* ```reason``` - Причина бана в виде строки. Если бана нет, поле не вернется
* ```date``` - Дата по UTC в формате YYYY-MM-DD HH:MM:SS. Если бана нет, поле не вернется
