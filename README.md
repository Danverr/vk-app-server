...
#### Параметры
* **toId** или **fromId** *(обязательно)* - VK ID пользователя

Имя ключа обозначает ориентацию запрашиваемых ребер в графе доступа. **toId** - все входящие ребра, **fromId** - все выходящие. Учтите, что параметр в данном запросе может быть только один.
#### Ответ
