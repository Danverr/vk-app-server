...
#### Ответ
Также, как и в ```GET /entries/```

## GET /entries/stats
Возвращает статистику пользователя в порядке убывания даты. Используйте вместо **GET /entries/**, чтобы избежать утечки приватных данных.
#### Параметры
* **userId** *(обязательно)* - VK ID пользователя
...
#### Параметры
* **csv** *(обязательно)* - Строка с указанными значениями через запятую. Можно указать параметры для нескольких записей сразу.
    ...
    * **isPublic** *(обязательно)* - Доступна ли запись друзьям, 1 или 0
    * **title** *(обязательно)* - Заголовок записи, строка с максимальной длиной в 64 символа
    * **note** *(обязательно)* - Текст записи, строка с максимальной длиной в 2048 символов
    * **date** *(обязательно)* - Дата по UTC в формате YYYY-MM-DD HH:MM:SS
#### Пример параметра csv
```
1,1,1,"title1","note1",0,"2020-06-10 09:00:00", // Строки тоже разделены запятой!
2,2,2,"title2","note2",1,"2020-06-11 10:00:00"
```
#### Ответ
В случае успеха вернет объект со значением ID последней добавленной записи и кол-вом добавленных записей
#### Пример ответа
```
{
    "rowsAdded": 1,
