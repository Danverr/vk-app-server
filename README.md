...
#### Ответ
В случае успеха вернет кол-во застронутых строк
...
* ```lastId``` - ID записи, после которой надо выбрать строки, т.е. вернутся поля с ```entryId < lastId```
...
#### Параметры
* ```time``` *(обязательно)* - Время по UTC, в которое должны приходить напоминания
#### Ответ
В случае успеха вернет кол-во затронутых строк
